...
class BaseRequest:
    ...
    def request(self, payload) -> dict:
        # TODO: add endpoint to the payload here or elsewhere?
        response = self.runtime_sm_client.invoke_endpoint(
            EndpointName=self.model,
            ContentType=self.content_type,
            Body=json.dumps(payload),
        )
        # check if this is necessary, how the invoke behaves
...
class Create(BaseRequest):
    ...
    def __init__(self, model: str = "lyra-fr"):
    ...
    def __call__(
        ...
        payload = {"text": text, "params": params, "endpoint": "create"}
...
class Analyse(BaseRequest):
    ...
    def __call__(
        ...
        request_id: str,
            ID string for the request.
        """
        payload = {"text": text, "endpoint": "analyse"}
        response = self.request(payload)
        request_id = response.get("request_id")
        cost = response.get("costs")
        outputs = response.get("outputs")[0]
        return outputs, cost, request_id
...
class Embed(BaseRequest):
    ...
    def __call__(
        self, text: Union[str, List[str]]) -> Tuple[List, int, str]:
        """Parameters
        -------------
        text: Union[str, List[str]],
            input text or list of input texts.
        ...
        request_id: str,
            ID string for the request.
        """
        payload = {"text": text, "endpoint": "embed"}
        response = self.request(payload)
        request_id = response.get("request_id")
        cost = response.get("costs")
        outputs = response.get("outputs")[0]
...
class Compare(BaseRequest):
    ...
    def __call__(
        ...
        request_id: str,
            ID string for the request.
        """
        payload = {"reference": reference, "candidates": candidates, "endpoint": "compare"}
        response = self.request(payload)
        request_id = response.get("request_id")
        cost = response.get("costs")
        outputs = response.get("outputs")[0]
...
class Tokenize(BaseRequest):
    ...
    def __call__(self, text: Union[str, List[str]]) -> Tuple[List, int, str]:
        ...
        request_id: str,
            ID string for the request.
        """
        payload = {"text": text, "endpoint": "tokenize"}
        response = self.request(payload)
        request_id = response.get("request_id")
        cost = response.get("costs")
        outputs = response.get("outputs")[0]
